...
object HH3DParser {
    ...
    fun parseDetail(html: String, mainUrl: String, pageUrl: String): DetailItem {
        ...
        val tags = extractGenres(html)
        val episodes = anchorRegex.findAll(html).mapNotNull { match ->
            val href = match.groupValues[1].trim()
            val ep = Regex("""(?i)/(?:[^/?#]+)/((?:vietsub)|(?:thuyet-minh))/tap-([0-9]+)(?:$|[/?#])""").find(href) ?: return@mapNotNull null
            val mode = ep.groupValues[1].lowercase()
            val number = ep.groupValues[2].toIntOrNull()
            val dubbed = mode == "thuyet-minh"
            EpisodeItem(
                name = "Tập ${number ?: stripTags(match.groupValues[2]).trim()} • ${if (dubbed) "Thuyết minh" else "Vietsub"}",
    ...
    fun slugify(value: String): String = normalizeForSearch(value).replace(' ', '-')
    ...
    private fun isDetailHref(href: String): Boolean {
        ...
        val clean = if (path.startsWith("http", true)) path.substringAfter("//").substringAfter('/', "") else path.trim('/')
        if (clean.isBlank() || '/' in clean) return false
        val blocked = setOf("dang-nhap", "dang-ky", "lien-he", "sitemap.xml")
        return clean.lowercase() !in blocked
    }

    private fun cleanCardTitle(value: String): String = value
        .replace(Regex("(?i)^(Vietsub|Thuyết minh)(,\\s*Thuyết minh)?\\s*-?\\s*"), "")
        .replace(Regex("(?i)^(Full\\s*HD|HD)\\s*"), "")
        .trim()
    ...
    private fun extractGenres(html: String): List<String> {
        val block = Regex("""(?is)Thể\s*loại\s*:(.*?)(?:\d(?:\.\d)?\s*/\s*10|Tập\s*mới\s*nhất|Nội\s*dung\s*phim)""").find(html)?.groupValues?.getOrNull(1) ?: return emptyList()
        return anchorRegex.findAll(block).map { stripTags(it.groupValues[2]).trim() }.filter { it.isNotBlank() }.distinct().toList()
    }
